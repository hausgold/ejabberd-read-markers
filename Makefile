...
.disable-module-conf:
	...
	@$(SED) 's/^\(\s*\)\(mod_read_markers:.*\)/\1# \2/g' \
...
.update-build-number:
	@$(eval VERSION = $(shell \
		$(GREP) -oP '\d+\.\d+\.\d+-\d+' include/mod_read_markers.hrl))
	...
	@$(SED) -i 's/$(VERSION)/$(NEXT_VERSION)/g' include/mod_read_markers.hrl
...
relevant-logs:
	...
	@$(COMPOSE) logs -f --tail='0' | $(GREP) --line-buffered '\[RM\]'
...
clean-database: .database
	...
	@$(call run-shell,PGPASSWORD=postgres $(PSQL) $(DATABASE) postgres -c \
		"TRUNCATE TABLE archive CASCADE; \
		 ...
		 TRUNCATE TABLE read_messages CASCADE; \
